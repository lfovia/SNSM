...
class VLAD:
    ...
    def can_use_cache_vlad(self):
        ...
        if not os.path.exists(self.cache_dir):
            return False
        if os.path.exists(f"{self.cache_dir}"):
            return True
        else:
            return False
    ...
    def fit(self, train_descs: Union[np.ndarray, torch.Tensor, None]):
        ...
        if self.can_use_cache_vlad():
            print("Using cluster centers from h5 file provided")
            
            with h5py.File(self.cache_dir, mode='r') as h5:
                self.c_centers = torch.tensor(h5.get("centroids")[...]) #torch.load(f"{self.cache_dir}/c_centers.pt")
                self.c_centers = self.c_centers.to('cuda')  
    ...
    def generate(self, query_descs: Union[np.ndarray, torch.Tensor],
        ...
        if self.vlad_mode == 'hard':
            ...
            used_clusters = set(labels.detach().cpu().numpy())
            ...
            if cache_id is not None and self.can_use_cache_vlad() \
                ...
                #print(query_descs.shape, self.c_centers.shape); print(hi)
    ...
    def generate_res_vec(self, 
        ...
        if cache_id is not None and self.can_use_cache_vlad() and \
            ...
            #print('devices:', query_descs.get_device(), self.c_centers.get_device()); print(hi)    
