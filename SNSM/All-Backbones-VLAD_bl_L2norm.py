...
def VLAD_descs(db_feat_loc, opt, cache_dir):
    vlad = VLAD(num_clusters=opt.num_clusters, cache_dir=cache_dir)#@@cache_dir=None)  
    Re_arrange2 = Rearrange('b (w c) -> b w c', w=opt.split_factor**2) 
    #print('db_feat_loc:', db_feat_loc.shape); print(hello)
    db_feat_loc = Re_arrange2(db_feat_loc)
    vlad.fit(None)#db_feat_loc.view(-1, db_feat_loc.shape[-1])) 
        ...
        for m in range(H):
           for n in range(W):
               ...
               self.similarity_map[m,n] = np.sum(
                                     ...
                                     cdist(self.input_image[m,n].detach().cpu().numpy(), self.input_image[m+k, n+l].detach().cpu().numpy()).item()
                                     )
                                     for k in [-1,1] ] 
                                     for l in [-1,1] ]  
                                      )
...
def dino_feature_extract(eval_set, model, device, opt, config, file_name, pool_size, cache_dir):
...
def main():
    ...
    parser.add_argument('--result_save_folder', type=str, default='./ICASSP-Ablation-Exps/CDist/')
    ...
    qfeats_loc = dino_feature_extract(qImgs, model, opt.device, opt, config, 'qfeats_dino_vlad.npy' , None, opt.cache_dir_thr) 
    dbfeats_loc = dino_feature_extract(dbImgs, model, opt.device, opt, config, 'dbfeats_dino_vlad.npy', None, opt.cache_dir_rgb) 
    
    #qfeats_loc = np.load('./ICASSP-Ablation-Exps/CDist/DINOv1b8/SNSM/M3FD/scale-4_patchSize_2/thr_patchfeats.npy')
    #dbfeats_loc = np.load('./ICASSP-Ablation-Exps/CDist/DINOv1b8/SNSM/M3FD/scale-4_patchSize_2/rgb_patchfeats.npy')
    
    feature_match(qfeats_loc, dbfeats_loc, dataset, opt.device, opt, config, opt.recall_file)
    
    #if not exists(opt.result_save_folder):
    #    makedirs(opt.result_save_folder)
    #np.save(opt.result_save_folder+'thr_patchfeats.npy', qfeats_loc)
    #np.save(opt.result_save_folder+'rgb_patchfeats.npy', dbfeats_loc)
    
    
    
    #preds = Match(qfeats, dbfeats, config)
